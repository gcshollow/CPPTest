...
#include "BeJwrMcxxMylLzCkGAA_0HANC.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<list<float64> vals>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("vals");
        TypeOffset t(MY_OFFSETOF(vals_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "vals") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, vals_);
   else
     istr >> vals_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, vals_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "vals=", get_vals()) ;
    ostr << '}';
}
