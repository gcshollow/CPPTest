...
#ifndef BEJWRMCXXMYLLZCKGAA_0HANC_H_
#define BEJWRMCXXMYLLZCKGAA_0HANC_H_
...
#define SELF BeJwrMcxxMylLzCkGAA_0HANC
...
public:
    ...
    typedef SPL::list<SPL::float64 > vals_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), vals_(ot.vals_) 
      { constructPayload(ot); }
    SELF(const vals_type & _vals) : Tuple(), vals_(_vals) { }
    ...
    vals_type & get_vals() { return vals_; }
    const vals_type & get_vals() const { return vals_; }
    void set_vals(const vals_type & _vals) { vals_ = _vals; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<vals_type >()(vals_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMCXXMYLLZCKGAA_0HANC_H_ 
