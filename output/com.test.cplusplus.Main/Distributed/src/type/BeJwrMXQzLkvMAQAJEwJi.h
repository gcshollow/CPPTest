...
#ifndef BEJWRMXQZLKVMAQAJEWJI_H_
#define BEJWRMXQZLKVMAQAJEWJI_H_
...
public:
    ...
    typedef SPL::float64 val_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), val_(ot.val_) 
      { constructPayload(ot); }
    SELF(const val_type & _val) : Tuple(), val_(_val) { }
    ...
    val_type & get_val() { return val_; }
    const val_type & get_val() const { return val_; }
    void set_val(const val_type & _val) { val_ = _val; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<val_type >()(val_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMXQZLKVMAQAJEWJI_H_ 
