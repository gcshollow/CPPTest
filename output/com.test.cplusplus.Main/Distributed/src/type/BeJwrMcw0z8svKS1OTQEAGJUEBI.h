...
#ifndef BEJWRMCW0Z8SVKS1OTQEAGJUEBI_H_
#define BEJWRMCW0Z8SVKS1OTQEAGJUEBI_H_
...
#define SELF BeJwrMcw0z8svKS1OTQEAGJUEBI
...
public:
    ...
    typedef SPL::int32 notused_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), notused_(ot.notused_) 
      { constructPayload(ot); }
    SELF(const notused_type & _notused) : Tuple(), notused_(_notused) { }
    ...
    notused_type & get_notused() { return notused_; }
    const notused_type & get_notused() const { return notused_; }
    void set_notused(const notused_type & _notused) { notused_ = _notused; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<notused_type >()(notused_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMCW0Z8SVKS1OTQEAGJUEBI_H_ 
