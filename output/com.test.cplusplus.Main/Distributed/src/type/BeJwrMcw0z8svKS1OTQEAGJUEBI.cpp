...
#include "BeJwrMcw0z8svKS1OTQEAGJUEBI.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<int32 notused>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("notused");
        TypeOffset t(MY_OFFSETOF(notused_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (s != "notused") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, notused_);
   else
     istr >> notused_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, notused_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
{
    ostr << '{';
    SPL::serializeWithPrecision(ostr << "notused=", get_notused()) ;
    ostr << '}';
}
