...
#ifndef BEJWRMCXXM87MKWMADD4CDY_H_
#define BEJWRMCXXM87MKWMADD4CDY_H_
...
public:
    ...
    typedef SPL::list<SPL::float64 > inv_type;

    enum { num_attributes = 1 } ;
    
    SELF() : Tuple() {}
    SELF(const Self & ot) : Tuple(), inv_(ot.inv_) 
      { constructPayload(ot); }
    SELF(const inv_type & _inv) : Tuple(), inv_(_inv) { }
    ...
    inv_type & get_inv() { return inv_; }
    const inv_type & get_inv() const { return inv_; }
    void set_inv(const inv_type & _inv) { inv_ = _inv; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<inv_type >()(inv_);
        return s;
    }
...
#undef SELF
#endif // BEJWRMCXXM87MKWMADD4CDY_H_ 
